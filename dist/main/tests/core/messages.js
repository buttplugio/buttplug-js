...
describe("Message", function () {
    ...
    it("Converts ok message from json correctly", function () {
        var jsonStr = '[{"Ok":{"Id":2}}]';
        chai_1.expect(MessageUtils_1.FromJSON(jsonStr)).to.deep.equal([new Messages.Ok(2)]);
    });
    ...
    it("Converts DeviceAdded message from json correctly", function () {
        ...
        chai_1.expect(MessageUtils_1.FromJSON(jsonStr)).to.deep.equal([new Messages.DeviceAdded(0, "Test", ["Ok", "Ping"])]);
    });
    it("Converts Error message from json correctly", function () {
        var jsonStr = '[{"Error":{"Id":2,"ErrorCode":3,"ErrorMessage":"TestError"}}]';
        chai_1.expect(MessageUtils_1.FromJSON(jsonStr)).to.deep.equal([new Messages.Error("TestError", Messages.ErrorClass.ERROR_MSG, 2)]);
    });
    it("Throws an error when trying to parse a message that breaks schema", function () {
        ...
        chai_1.expect(MessageUtils_1.FromJSON(jsonStr)[0].constructor.name).to.equal("Error");
    });
    it("Handles KiirooCmd messages correctly", function () {
        var jsonStr = '[{"KiirooCmd":{"Id":1,"DeviceIndex":0,"Command":"3"}}]';
        chai_1.expect(MessageUtils_1.FromJSON(jsonStr)[0].constructor.name).to.equal("KiirooCmd");
