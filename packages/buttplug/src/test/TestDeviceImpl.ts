...
export class TestDeviceImpl extends ButtplugDeviceImpl {
  ...
  public async ReadValueInternal(aOptions: ButtplugDeviceReadOptions): Promise<Buffer> {
  ...
  public async WriteValueInternal(aData: Buffer, aOptions: ButtplugDeviceWriteOptions): Promise<void> {
    this._lastValueWritten.push([aOptions.Endpoint, aData]);
    return Promise.resolve();
  }

  public async SubscribeToUpdatesInternal(aOptions: ButtplugDeviceReadOptions): Promise<void> {
    return Promise.resolve();
  }
