...
export class ButtplugClientDevice {
  ...
  private CheckGenericSubcommandList<T extends Messages.GenericMessageSubcommand>(aType: string, aCmdList: T[], aLimitValue: number) {
    if (aCmdList.length == 0 || aCmdList.length > aLimitValue)
    {
      if (aLimitValue == 1)
      {
        throw new ButtplugDeviceException(`${aType} requires 1 subcommand for this device, ${aCmdList.length} present.`);
      }

      throw new ButtplugDeviceException(`${aType} requires between 1 and ${aLimitValue} subcommands for this device, ${aCmdList.length} present.`);
    }

    for (var cmd of aCmdList)
    {
  ...
  public async SendVibrateCmd(aSpeed: number | number[]): Promise<void> {
    ...
    if (typeof(aSpeed) === "number") {
      ...
      msg = Messages.VibrateCmd.Create(this._index,
                                       new Array(this.MessageAttributes(Messages.VibrateCmd.name).FeatureCount).fill(aSpeed));
  ...
  public async SendRotateCmd(aValues: number | [number, boolean][], aClockwise?: boolean): Promise<void> {
    ...
    if (typeof(aValues) === "number") {
      ...
      msg = Messages.RotateCmd.Create(this._index,
                                      new Array(this.MessageAttributes(Messages.RotateCmd.name).FeatureCount).fill([aValues, aClockwise]));
      ...
      throw new ButtplugDeviceException("SendRotateCmd can only take a number and boolean, or an array of number/boolean tuples");
    }
    await this.SendMessageAsync(msg);
  }
  ...
  public async SendLinearCmd(aValues: number | [number, number][], aDuration?: number): Promise<void> {
    ...
    if (typeof(aValues) === "number") {
      ...
      msg = Messages.LinearCmd.Create(this._index,
                                      new Array(this.MessageAttributes(Messages.LinearCmd.name).FeatureCount).fill([aValues, aDuration]));
    } else if (Array.isArray(aValues)) {
      msg = Messages.LinearCmd.Create(this._index,
                                      aValues);
      this.CheckGenericSubcommandList(Messages.SpeedSubcommand.name,
      ...
      throw new ButtplugDeviceException("SendLinearCmd can only take a number and number, or an array of number/number tuples");
    }
    await this.SendMessageAsync(msg);
  }
